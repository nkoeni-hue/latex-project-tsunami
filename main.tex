...
\begin{figure}[htbp]
  ...
  \caption{Wavemaker displacement for the broken and unbroken wave.}
...
Force time histories were decomposed with Butterworth filters: high-pass filtering isolated short-duration impact peaks, while low-pass filtering captured sustained damming forces (Figure \ref{fig:high_low_pass}). A fourth-order Butterworth filter with a cutoff frequency of 2.5 Hz was applied. These parameters were selected based on preliminary analyses, which showed that this most effectively isolates the damming . Examples (Figures \ref{fig:timehist_combined}–\ref{fig:timehist_damming_combined}) show the applied filter to the 1L case and the 24L case. 
The repeatability of unbroken solitary waves provided a consistent baseline for isolating and evaluating debris-induced loading mechanisms.
...
\begin{figure}[ht]
    ...
    \caption{Normalized force responses for the In Air/Pendulum test and 1L configurations. Each subplot is normalized by its respective maximum force.}
...
\hline
\end{tabular}
\caption{Median natural frequencies and damping ratios (\% and absolute) for each condition.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{QuadraticFit_FirstPeak_AllData.png}
    \caption{Quadratic fit through all normalized first-peak values. Scatter points represent individual trial measurements, and the black line shows the quadratic trend.}
    \label{fig:quadraticFit_firstPeak}
\end{figure}
