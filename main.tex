...
% \author{Noemie Koenig, Marc O. Eberhard, Pedro Arduino, Micheal Motley, Dakota Mascarenas}
\date{\today}
...
Longitudinal debris alignments (i.e., oriented parallel to the incoming flow) tend to produce larger impacts than transverse configurations, while random fields display greater variability in impact magnitudes. Similarly, single-size debris groups behave differently than multi-size fields, particularly in their ability to form blockages and generate damming forces. Finally packing ratio, defined as the ratio of tightly packed debris area to the total spread area, controls the degree of clogging and thus the overall magnitude of structural loading.
...
\begin{figure}[htbp]
    ...
    \caption{Filtered and unfiltered force signals demonstrating the effect of a 4th-order Butterworth filter (cutoff = 2.5 Hz) used for force decomposition.} 
    \label{fig:high_low_pass} 
\end{figure}

Force time histories were decomposed with Butterworth filters: high-pass filtering isolated short-duration impact forces, while low-pass filtering captured sustained damming forces (Figure \ref{fig:high_low_pass}). A fourth-order Butterworth filter with a cutoff frequency of 2.5 Hz was applied. These parameters were selected based on preliminary analyses, which showed that this most effectively isolates the damming reaction forces. Examples (Figures \ref{fig:timehist_combined}–\ref{fig:timehist_damming_combined}) show the applied filter to the 1 L case and the 24 L case. 
...
Signal peaks were identified within predefined time intervals, determined from video observations of the debris motion. The first impact was detected within the initial time window before 34.2 s, while subsequent impacts were identified by considering the highest impacts after the first impact and excluding the first impact. 
...
The second phase corresponds to sustained debris accumulation and flow blockage (damming), which was captured through low-pass filtering. The normalized damming force was computed using:
...
All five single-piece configurations (10 trials each) were conducted to evaluate the effect of debris size on the reaction force. These tests aimed to characterize the behavior of individual debris pieces before examining group effects. Figure~\ref{fig:firstpeak_regular_size} shows the reaction force measured for single debris pieces with varying orientations. Higher variability is observed in the transverse orientation. 

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{FirstPeak_Regular_SplitByTrial_single.png}
    \caption{Comparison of debris size at the first impact for regular configurations. Baseline for Normalization as indicated is the 1L median impact.}
    \label{fig:firstpeak_regular_size}
\end{figure}


\section{Group effects}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[t]{0.9\textwidth}
        ...
        \caption{Total number of debris pieces within the flume.}
    ...
    \begin{subfigure}[t]{0.9\textwidth}
        ...
        \caption{Debris count of pieces causing structural impact.}
        \label{fig:firstpeak_regular_remap}
    \end{subfigure}
    \caption{First impact forces in regular configurations for longitudinal and transverse orientations.}
...
\begin{figure}[htbp]
    ...
    \caption{First impact forces in random debris fields. Single-size debris yields higher impact forces than multi-size debris; lower packing ratio reduces impacts.}
...
\begin{figure}[htbp]
    ...
    \caption{Median normalized damming reaction forces in random fields, adjusted for debris field width. Forces are roughly 100\% higher than wave-only loading, with packing ratio effects stronger in single-size fields and weaker in multi-size fields.}
