...
The internal frame used hollow steel sections for stiffness, with additional aluminum-encased stiffeners to improve rotational resistance and load transfer. Eight load cells were installed: two streamwise, two transverse, and four vertical pancake cells, all sampled at 1200 Hz. Steel legs, 0.25 m in height, were placed below the structure but were not load bearing. Front and back view of the structure can be seen in Figure \ref{fig:teststructure_views}.
 ...
 \begin{figure}[htbp]
    ...
    The front is the exposed face subjected to debris impact, while the back shows the instrumentation and load cell mounting.}
    \label{fig:teststructure_views}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{Configurations.pdf}
...
Signal peaks were identified within predefined time intervals, determined from video observations of the debris motion. The first impact was detected within the initial time window, while subsequent impacts were identified by considering only times after the first impact and excluding the first impact. 
...
\begin{figure}[htbp]
    ...
    \caption{Comparison of debris size at the first impact for regular configurations. The debris count in initial configurations is indicated within the boxplots.}
...
\begin{figure}[htbp]
    ...
    \caption{Median normalized first impacts in random fields, adjusted by structure width. Single-size debris fields produce higher impacts than multi-size fields. In multi-size fields, filtering out large pieces shows that smaller debris adds modest additional impact.}
