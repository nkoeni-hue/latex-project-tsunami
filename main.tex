...
A total of 32 in-air swing tests were conducted to characterize the dynamic behavior of the test structure. Each test involved a single debris piece of identical dimensions, released from varying drop-off heights. Frequency spectra obtained through Fast Fourier Transform (FFT) analysis revealed three dominant amplification peaks, identified as the principal modes of the structure, as summarized in Table \ref{tab:mode_summary}. The same modes were subsequently confirmed for the 1 L configuration in water, where the natural frequencies decreased as expected due to potential added mass contributions. Isolating the modes in the reaction force signal further allowed the determination of damping ratios for each mode through logarithmic decrement analysis, revealing an increase in damping when the structure was submerged, consistent with additional energy dissipation from fluid–structure interaction.
...
A total of 5 single-piece configurations (10 trials each) have been conducted in order to evaluate the effect of debris size on the Reaction Force.  
