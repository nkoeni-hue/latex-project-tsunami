...
The results reported in Table \ref{tab:mode_summary} show that the identified modal frequencies of the structure in water were lower than those identified for the in-air tests.  This change is consistent with increased resistance provided by the surrounding water.  Note that the percentage frequency difference was larger for the higher modes than the lower modes, consisting of decreases of 6\%, 26\% and 2\% for modes 1, 2 and 3. The percentage frequency difference between In-Air and In-Water conditions is calculated as the change in frequency relative to the In-Air value: 
$\Delta f_{\%} = (f_{\text{In-Water}} - f_{\text{In-Air}})/f_{\text{In-Air}} \times 100\%$.

Table \ref{tab:mode_summary} also reports the damping ratios identified for the In-Air and In-Water tests.  The damping ratio tended to decrease with increasing mode order, and the ratio was consistently larger for the In-Water tests than the In-Air tests.  The damping ratios increased substantially when the structure was tested in water. The percentage increase in damping was approximately 160\%, 370\%, and 80\% for modes 1, 2, and 3, respectively. The second mode showed the greatest sensitivity to the surrounding fluid.

Note also that the modal properties for the In-Air tests varied less than those identified for the In-Water tests. This increased variability is consistent with previous observations that fluid–structure interactions introduce greater uncertainty and variability into tests compared to tests conducted under In-Air conditions.
...
Random debris fields with irregular orientations are analyzed by impact and damming forces as functions of debris count and packing ratio. Debris was initially arranged within a rectangular frame; at lower packing ratios, increasing the debris count caused the frame to extend beyond the structure’s width. Subsequent adjustments account for this using video-derived impact probabilities. The analysis includes adjustments to account for debris field width and a size-based assessment that isolates the contributions of the largest debris pieces in multi-size fields, revealing the limited influence of smaller pieces on structural loading.

\subsection*{Impact Reaction Forces} 
Figure~\ref{fig:random_peaks_first} shows the impact reaction forces from the first impact,  separated by packing ratio. Boxplot whiskers indicate the minimum and maximum values, the boxes show the interquartile range (IQR), and the solid line within each box marks the median value.
...
Figure~\ref{fig:random_peaks_adjusted} shows the the width-based adjustment applied to the single-sized debris tests. After taking debris field width into account, a linear trend is visible again. This underscores the previous observation that the first impact reaction force scales linearly with debris mass. Adjustment for debris field width also attenuates the effects of packing ratio previously observed in Figure~\ref{fig:random_peaks_first}. Figure~\ref{fig:random_first_median_adjusted} shows the median first impact reaction of single and multi-sized fields after adjustment. Single-sized debris fields produce higher forces than multi-sized fields. Multi-size fields consistently yielded lower impact magnitudes because smaller pieces contributed less momentum . The difference between multi-sized vs. single-sized scenarios became more relevant with larger debris groups, as multiple debris pieces struck separately rather than together.
...
\section{A Model for first impact reaction force}
The authors propose the following regression model for the 
\begin{equation}
F_\mathrm{pred}(N, \text{Type}, \text{Density}) 
= 1 + (N-1) \,\cdot\, C_L \,\cdot\, k_\mathrm{orientation} \,\cdot\, k_\mathrm{Type} \,\cdot\, k_\mathrm{PackingRatio}
\end{equation}
...
L
 ...
 is computed from regular L-orientation trials, while orientation, random, type (Single/Multi), and density factors scale this slope for other conditions.

The model predictions are plotted alongside the measured data, with error bars representing the standard error of the mean (SEM) for each condition. These error bars indicate the variability of the median first impact estimate across trials, providing a measure of uncertainty in the observed values. The regression lines illustrate the hierarchical model predictions, accounting for orientation, type, and density adjustments.
% ================= Predicted vs Measured =================
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/PredVSMeas.png}
    \caption{Predicted versus measured $F(N)/F(1)$ using the hierarchical factor model. The diagonal line represents perfect agreement between predicted and measured values.}
    \label{fig:PredVSMeas}
\end{figure}

% ================= Regression for Regular L and T =================
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/RegressionTandL.png}
    \caption{Regression of regular L- and T-orientation trials. Markers show measured median first impacts with error bars (standard error of the median), and lines show the hierarchical factor model fit.}
...
\begin{figure}[h]
    ...
    \caption{Regression for Multi-type trials. Markers show measured median first impacts with error bars. Solid lines indicate baseline fits, dashed lines density-adjusted fits for low-density trials.}
    \label{fig:RegressionMulti}
\end{figure}

% ================= Regression for Single =================
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/RegressionSingle.png}
    \caption{Regression for Single-type trials. Markers show measured median first impacts with error bars. Solid lines indicate baseline fits, dashed lines density-adjusted fits for low-density trials.}
